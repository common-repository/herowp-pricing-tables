...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Hero Pricing Tables', 'bbold');
			$defaults['page_title'] = __('Hero Pricing Tables', 'bbold');
			$defaults['page_slug'] = __('hero-pricing-tables', 'bbold');
		...
		function admin_pages() {
			...
			$this->page = add_menu_page( $this->args['page_title'], $this->args['menu_title'], 'manage_options', $this->args['page_slug'], array(&$this, 'builder_page_show'));
		...
		function add_admin_bar(){
			global $wp_admin_bar;
			$wp_admin_bar->add_menu( array( 'id' => 'hero-pricing-tables', 'parent' => false, 'title' => 'HeroWP Pricing Tables', 'href' => admin_url('themes.php?page='.$this->args['page_slug']) ) );
		...
		function admin_enqueue() {
			...
			wp_register_style( 'font-awesome-css', $this->url.'assets/css/font-awesome.min.css', array(), time(), 'all');
		...
		function view_enqueue() {
			...
			wp_register_style( 'bootstrap-light-css', $this->url.'assets/css/bootstrap-light.css', array(), time(), 'all');
			wp_enqueue_style('bootstrap-light-css');			
			wp_register_style( 'pricing-tables-css', $this->url.'assets/css/pricing-tables.css', array(), time(), 'all');
			wp_enqueue_style('pricing-tables-css');
			wp_register_style( 'font-awesome-css', $this->url.'assets/css/font-awesome.min.css', array(), time(), 'all');
			wp_enqueue_style('font-awesome-css');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'bbold') .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'bbold');
				echo '</p>';
				return;
		...
		function create_template($title) {
			...
			if(!get_page_by_title( $title, 'OBJECT', 'template' )) {
				...
				return new WP_Error('duplicate_template', 'Pricing table names must be unique, try a different name');
			}
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', 'bbold');
				echo '</p>';
				
			} else {
				//template wrapper
				echo '
<div id="aq-template-wrapper-'.$template_id.'" class=""><!--AQ TEMPLATE WRAPPER-'.$template_id.' START-->';
				...
				echo '
</div><!--AQ TEMPLATE WRAPPER-'.$template_id.' END-->';
			}
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'pricing_layout', $shortcode_tags ) ) {
				add_shortcode( 'pricing_layout', array(&$this, 'do_shortcode') );
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'bbold') .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . HEROWP_DIR . '/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'bbold' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'bbold' )  . '">' . $img . '</a>';
				} else {
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . HEROWP_DIR . '/assets/images/aqua-media-button.png ); margin-top: -1px;"></span>';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'bbold' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'bbold' ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'bbold' ) ); ?>");
							return;
						}

						/** Send shortcode to editor */
						window.send_to_editor('[pricing_layout id="' + id + '"]');
					}
				</script>
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'bbold' ), '<a href="' .admin_url().'themes.php?page=aq-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Page Template', 'bbold' ); ?></h3><br />
						...
						<input type="button" id="aqpb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'bbold' ); ?>" onclick="insertTemplate();" />
						<a id="aqpb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'bbold' ); ?>"><?php echo esc_attr__( 'Cancel', 'bbold' ); ?></a>
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:','bbold') . '</strong></p>'			
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview','bbold'),
		...
		function builder_page_show(){
			...
			require_once(HEROWP_PATH . 'view/view-builder-page.php');
