	...
	class AQ_Block {
		...
		var $fa_icon;
	 	...
	 	function block($instance) {
	 		extract($instance);
	 		echo __('function AQ_Block::block should not be accessed directly. Output generated by the ', 'bbold') . strtoupper($id_base). ' Class';
	 	}
	 	...
	 	function form($instance) {
	 		echo '<p class="no-options-block">' . __('There are no options for this block.', 'bbold') . '</p>';
	 		return 'noform';
	 	}
	 	...
	 	function before_form($instance) {
	 		...
	 		echo '<li id="template-block-'.$number.'" class="block block-'.$id_base.' '. $size .' '.$resizable.' '.$class_css.'">',
	 				'<dl class="block-bar">',
	 					'<dt class="block-handle">',
	 						'<div class="block-title">',
	 							'<i class="'.$fa_icon.'"></i> '.$name , $title, 
	 						'</div>',
	 	...
	 	function before_block($instance) {
			...
			//var_dump($instance);
			if ($number==1 || $instance["parent"]==0)
			echo '
      <div id="aq-block-'.$template_id.'-'.$number.'" class="col-md-'.$size.' nopadding"><!--AQ BLOCK-'.$template_id.'-'.$number.' START-->';
			else
	 		echo '
      <div id="aq-block-'.$template_id.'-'.$number.'" class="col-md-'.$size.'" style="padding:0"><!--AQ BLOCK-'.$template_id.'-'.$number.' START-->';
	 	...
	 	function after_block($instance) {
	 		extract($instance);
	 		echo '
      </div><!--AQ BLOCK-'.$template_id.'-'.$number.' END-->
